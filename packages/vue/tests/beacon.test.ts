...
describe("fireBeacon", () => {
  ...
  it("fires beacon with correct URL for a publishable token", () => {
  ...
  it("deduplicates: second call for same token is a no-op", () => {
    fireBeacon("qt_dup");
    fireBeacon("qt_dup");
  ...
  it("skips sk_ tokens (server keys)", () => {
    fireBeacon("sk_secret_key");

    expect(imageSrcs).toHaveLength(0);
  });

  it("skips when typeof window is undefined (SSR)", () => {
